 ...
 *
 *  Your index.html is a SEPARATE static site.
 *  It should fetch to your server URL e.g.:
 *    fetch("https://your-server.onrender.com/api/mobile/request-payment")
 *
 *  This server is a pure API proxy — it does NOT serve any HTML.
 *  It forwards every request to Pamodzi and returns the response.
 * ============================================================
 */
...
// Middleware
app.use(cors()); // Allow requests from your separate frontend (any origin)
...
function interpretCode(code) {
  ...
  if (!code) return "No errorCode in response — inspect the raw body above.";
  return map[code] || `Unrecognised code: ${code} — inspect the raw body for details.`;
}

// Extract errorCode from Pamodzi response — check all common field name variations
function extractCode(data) {
  if (!data || typeof data !== "object") return null;
  return data.errorCode    || data.ErrorCode    || data.error_code
...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    res.status(response.status).json({
      ...
      pamodzi: data,
      interpretation: interpretCode(extractCode(data)),
    });
  } catch (err) {
    handleError(res, err);
  }
